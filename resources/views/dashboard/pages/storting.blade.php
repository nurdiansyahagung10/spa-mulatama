@extends('dashboard.main') @section('dashboardheader')
    ...
    <div class="flex p-3 dark:text-white mt-10 mb-5 justify-between items-center">
        <h1 class="text-xl w-10/12">Table List Storting</h1>
            ...
            <div class="flex flex-col gap-4 -mx-3 mb-6 h-2">
                ...
                <div class="flex gap-2 px-4 w-full">
                    <div class="flex flex-row gap-1 w-full">
                        ...
                        <input type="date" name="tanggal" id="tanggal_storting"
                            class="block w-full bg-transparent darK:text-white border text-black dark:text-white border-stone-400 rounded-full py-[0.35rem] px-4 leading-tight focus:outline-none dark:focus:border-white">
                    </div>
        ...
        <table class="table  text-center align-middle">
            ...
            <tbody id="anggota-list-body-table" class="text-black/60 dark:text-stone-200">
                <script>
                    ...
                    function innertablestorting(index, item) {
                        return `
                    <tr class="dark:border-stone-400 dark:text-stone-300 hover:text-black dark:hover:text-white">
                        <td>${index + 1}</td>
                        <td>${item.dropping.anggota.nama}</td>
                        <td>${item.dropping.nominal_dropping / 10 }</td>
                        <td>${item.dropping.nominal_dropping / 10 * 0.1}</td>
                        <td>${item.dropping.nominal_dropping / 10 + item.dropping.nominal_dropping / 10 * 0.1}</td>
                        <td>${item.nominal_storting}</td>
                        <td>${item.nominal_storting - (item.dropping.nominal_dropping / 10 + item.dropping.nominal_dropping / 10 * 0.1) < 0 ? item.nominal_storting - (item.dropping.nominal_dropping / 10 + item.dropping.nominal_dropping / 10 * 0.1) : 0}</td>
            ...
            <tr class="border-b-0">
                <td colspan="${colspan}" class="h-[20rem]"><span class="text-black loading dark:text-white loading-bars loading-md"></span></td>
                    ...
                    window.addEventListener("load", async () => {
                        ...
                        newdata = data;
                        if (admin == false) {
                            newdata = data.filter(item => item.dropping.anggota.pdl.cabang.nama === getusercabang)
                        }

                        listtablebodyanggota.innerHTML = "";
                        newdata.forEach((item, index) => {
                            listtablebodyanggota.innerHTML += innertablestorting(index, item);
                        });
                    });
                    ...
                    document.getElementById('pdl').addEventListener('input', (e) => {
                        ...
                        listtablebodyanggota.innerHTML = "";
                        newdata.forEach((item, index) => {
                            if (item.dropping.anggota.pdl.nama == e.target.value) {

                                listtablebodyanggota.innerHTML += innertablestorting(index, item);
                            } else if (e.target.value == 'pdl') {
                                if (item.dropping.anggota.pdl.cabang.nama == cabang) {
                    ...
                    search.addEventListener("input", async (e) => {
                        ...
                        document.getElementById('tanggal_storting').value = '';
                        if (admin) {
               ...
               `;
                        ...
                        if (btnsaerchbyvalue.trim() == "Nama") {
                            newdata = data.filter((item) =>
                                item.dropping.anggota.nama.toLowerCase().includes(searchdata)
                            );
                        ...
                        if (admin == false) {
                            newdata = newdata.filter(item => item.dropping.anggota.pdl.cabang.nama === getusercabang)
                        }
            ...
            <option value="pdl">pdl</option>
             ...
             `;
               ...
               `;
                            ...
                            listtablebodyanggota.innerHTML = "";
                            newdata.forEach((item, index) => {
                                if (item.dropping.anggota.pdl.cabang.nama == e.target.value) {
                                    listtablebodyanggota.innerHTML += innertablestorting(index, item);
                    ...
                    document.getElementById('tanggal_storting').addEventListener("input", async (e) => {
